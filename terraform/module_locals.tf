...
locals {
  cert-manager = {
    ...
    docker_iamge_webhook    = coalesce(var.cert-manager.docker_iamge_webhook, "quay.io/jetstack/cert-manager-webhook")
    docker_image            = coalesce(var.cert-manager.docker_image, "quay.io/jetstack/cert-manager-controller")
    docker_image_acmesolver = coalesce(var.cert-manager.docker_image_acmesolver, "quay.io/jetstack/cert-manager-acmesolver")
    docker_image_injector   = coalesce(var.cert-manager.docker_image_injector, "quay.io/jetstack/cert-manager-cainjector")
    docker_image_statup     = coalesce(var.cert-manager.docker_image_statup, "quay.io/jetstack/cert-manager-startupapicheck")
  ...
  nifi-kop = {
    ...
    docker_tag         = coalesce(var.nifi-kop.docker_tag, "1.9.0")
  ...
  palworld = {
    ...
    helm_chart_name    = coalesce(var.palworld.helm_chart_name, "tailscale-operator")
    helm_chart_version = coalesce(var.palworld.helm_chart_version, "1.68.1")
    helm_release_name  = coalesce(var.palworld.helm_release_name, "tailscale-operator")
    helm_repo_url      = coalesce(var.palworld.helm_repo_url, "https://pkgs.tailscale.com/helmcharts")
