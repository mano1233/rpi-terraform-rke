resource "helm_release" "cert-manager" {
  ...
  values = [
    templatefile("${path.module}/helm-values.tfpl", {
      ...
      docker_image_webhook  = var.docker_image_webhook
      docker_image_injector = var.docker_image_injector
      docker_image_acme     = var.docker_image_acme
      docker_image_startup  = var.docker_image_startup
